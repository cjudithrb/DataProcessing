...
class TestParser():
	...
	def get_function_metadata(class_identifier, function_node, blob: str):
		...
		#Body # Judith/Miguel
		if "test" in str(class_identifier).lower():
			metadata['body'] = TestParser.match_from_span(function_node, blob)
		else:
			metadata['body'] = TestParser.match_from_span_comment_plus(function_node, blob)			
	...
	"""
	Concatena el comentario de un metodo con el cuerpo del metodo
	"""
	@staticmethod
	def match_from_span_comment_plus(node, blob: str) -> str:
		"""
		Extract the source code associated with a node of the tree
		"""
		comentario = ""

		if node.prev_sibling and node.prev_sibling.type=='block_comment' and node.prev_sibling.next_sibling and node.prev_sibling.next_sibling.id == node.id:
			comentario = str(node.prev_sibling.text)
